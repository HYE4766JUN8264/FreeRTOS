/***********************************************************************************************************************
 * This file was generated by the S32 Configuration Tools. Any manual edits made to this file
 * will be overwritten if the respective S32 Configuration Tools is used to update this file.
 **********************************************************************************************************************/

/* clang-format off */
/* TEXT BELOW IS USED AS SETTING FOR TOOLS *************************************
!!GlobalInfo
product: Peripherals v10.0
processor: S32K144
package_id: S32K144_LQFP100
mcu_data: s32sdk_s32k1xx_rtm_403
processor_version: 0.0.0
functionalGroups:
- name: BOARD_InitPeripherals
  UUID: 33afab84-3987-42cd-bca4-67126556733a
  called_from_default_init: true
  selectedCore: core0
 * BE CAREFUL MODIFYING THIS COMMENT - IT IS YAML SETTINGS FOR TOOLS **********/
/* clang-format on */

/*******************************************************************************
 * Included files 
 ******************************************************************************/
#include "peripherals_wdg_pal_1.h"

/*******************************************************************************
 * wdg_pal_1 initialization code
 ******************************************************************************/
/* clang-format off */
/* TEXT BELOW IS USED AS SETTING FOR TOOLS *************************************
instance:
- name: 'wdg_pal_1'
- type: 'wdg_pal'
- mode: 'general'
- custom_name_enabled: 'true'
- type_id: 'wdg_pal'
- functional_group: 'BOARD_InitPeripherals'
- peripheral: 'WDOG'
- config_sets:
  - wdg_pal:
    - wdg_pal_UserConfig:
      - wdg_pal_Config:
        - 0:
          - name: 'wdg_pal_1_Config0'
          - readonly: 'true'
          - clkSource: 'WDG_PAL_LPO_CLOCK'
          - opMode:
            - wait: 'false'
            - stop: 'false'
            - debug: 'true'
          - intEnable: 'true'
          - prescalerEnable: 'true'
          - winEnable: 'false'
          - percentWindow: '0'
          - timeoutValue: '10000'
 * BE CAREFUL MODIFYING THIS COMMENT - IT IS YAML SETTINGS FOR TOOLS **********/
/* clang-format on */

/**
 * @page misra_violations MISRA-C:2012 violations
 *
 * @section [global]
 * Violates MISRA 2012 Advisory Rule 8.7, External variable could be made static.
 * The external variables will be used in other source files in application code.
 *
 */

/* WDG PAL instance information */
const wdg_instance_t wdg_pal_1_Instance = {
  .instType = WDG_INST_TYPE_WDOG,
  .instIdx  = 0U
};

/* Serial User Configurations */
const wdg_config_t wdg_pal_1_Config0 = {
  .clkSource        = WDG_PAL_LPO_CLOCK,
  .opMode           = {
    .wait           = false,
    .stop           = false,
    .debug          = true
  },
  .intEnable        = true,
  .prescalerEnable  = true,
  .winEnable        = false,
  .percentWindow    = 0U,
  .timeoutValue     = 10000UL
};


